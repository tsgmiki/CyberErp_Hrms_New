...
public class EmployeeExperience : BaseEntity, IAggregateRoot, IAuditable
{
    ...
    /// <summary>
    /// Pay in the role, when known. NULLABLE on purpose: it is unknown for every row that already
    /// exists, and for most externally-entered history a candidate simply does not disclose it —
    /// storing 0 would assert "unpaid", which is a different claim from "not recorded".
    /// </summary>
