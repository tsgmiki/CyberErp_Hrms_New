...
import generateDocument from "@/services/admin/documentTemplate/generate";
...
// Print stylesheet injected into the print document. Beyond page margins it (1) keeps colours
// and backgrounds, and (2) forces any inline flexbox layout (e.g. a logo-left / text-right
// header) to stay on one row — some print engines otherwise collapse flex to a vertical stack.
const PRINT_STYLE = `
  @page { margin: 16mm; }
  @media print {
...
function GenerateDocumentModal({ employeeId, employeeName, onClose }: Props) {
  ...
  const templates = (templatePage?.data ?? []).filter(
    (x: DocumentTemplateModel) => x.isActive !== false,
  );
  ...
  const {
    ...
    queryKey: ["generateDocument", templateId, employeeId],
    queryFn: () => generateDocument(templateId, employeeId),
    enabled: !!templateId && !!employeeId,
  });

  const handlePrint = useReactToPrint({
    contentRef,
    documentTitle: generated?.title || "document",
  ...
  return (
    <Modal
      visible
      size="fullscreen"
      title={`${t("Generate Document")}${employeeName ? ` — ${employeeName}` : ""}`}
      onClose={onClose}
      footer={
        <>
          <button
        ...
        <aside className="w-60 shrink-0 space-y-1.5 overflow-y-auto border-r border-border pr-3">
          ...
          {!loadingTemplates && templates.length === 0 && (
            <p className="text-sm text-muted">
              No active templates. Create one under Document Templates first.
            </p>
          )}
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="block text-xs text-muted">
                  {documentTypeLabel(tpl.documentType)}
                </span>
              </span>
            </button>
          ))}
