...
import saveAwardCategory from "@/services/admin/awardCategory/save";
import getAwardCategory from "@/services/admin/awardCategory/get";
import Loading from "../../common/loader/loader";
import { activeStatusOptions, activeId, activeLabel } from "@/constants/orgStructure";

const FormProvider = memo(FormProviders);
const NEW_DEFAULTS: AwardCategoryModel = { isActive: true };

function AwardCategoryForm(props: { id: string; setId: (id: string) => void }) {
  const { id, setId } = props;

  const [formState, setFormState] = useState<any>({});
  const [isLoading, setIsLoading] = useState(false);
  const [formData, setFormData] = useState<AwardCategoryModel>({ ...NEW_DEFAULTS });
  const formRef = React.createRef<HTMLFormElement>();
  const queryClient = useQueryClient();

  const { data: record, isLoading: pending } = useQuery({
    queryKey: ["awardCategory", id],
    queryFn: () => getAwardCategory(id),
  ...
  const submitHandler = async (e: any) => {
    ...
    setIsLoading(true);
    const result = await saveAwardCategory(fd);
    setFormState(result);
    setIsLoading(false);
  };
  ...
  useEffect(() => {
    if (formState.status == "success") {
      ...
      // ["awardCategory", id] kept the pre-save copy and the client's 30 s staleTime served it to the
      // next Edit WITHOUT refetching -- grid fresh, form stale until a full page reload.
      queryClient.invalidateQueries({ queryKey: ["awardCategory"] });
      setId("");
    }
  }, [formState]);
    ...
    <div className="text-white">
      ...
      <FormProvider
        ...
        form={{
          ...
          SubmitButton: "top",
          components: [
            { name: "name", label: "Name", placeholder: "e.g. Service Excellence", required: true, value: formData.name, onChange: changeHandler, error: formState?.zodErrors?.name, type: "text" },
            {
            ...
            { name: "criteria", label: "Criteria", placeholder: "Shared eligibility criteria for awards in this category", value: formData.criteria, onChange: changeHandler, type: "textarea", colSpan: "full" },
