...
function EmployeeCareerPathForm({ id, setId }: { id: string; setId: (id: string) => void }) {
  ...
  const submit = async () => {
    ...
    if (result.status === "success") {
      ...
      // ["employeeCareerPath", id] kept the pre-save copy and the client's 30 s staleTime served it to the
      // next Edit WITHOUT refetching -- grid fresh, form stale until a full page reload.
      queryClient.invalidateQueries({ queryKey: ["employeeCareerPath"] });
