...
function WorkWeekConfigurationForm(props: { id: string; setId: (id: string) => void }) {
  ...
  useEffect(() => {
    if (formState.status == "success") {
      ...
      // ["workWeekConfiguration", id] kept the pre-save copy and the client's 30 s staleTime served it to the
      // next Edit WITHOUT refetching -- grid fresh, form stale until a full page reload.
      queryClient.invalidateQueries({ queryKey: ["workWeekConfiguration"] });
      setId("");
    }
  }, [formState]);
