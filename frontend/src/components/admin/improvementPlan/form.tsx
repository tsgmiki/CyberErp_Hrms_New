...
import { Plus, Trash2, Save, CheckCircle2, Flag } from "lucide-react";
...
function ImprovementPlanForm({ id, setId }: { id: string; setId: (id: string) => void }) {
  ...
  return (
    <div className="space-y-5 text-foreground">
      <form onSubmit={submit} className="space-y-5">
        <section className="rounded-lg border border-border bg-card p-4">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-semibold">{t("Plan Details")}</h3>
            {completed && (
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              ...
              <select className={INPUT} value={meta.employeeId ?? ""} disabled={completed} onChange={(e) => setMetaField("employeeId", e.target.value)} required>
            ...
            <div className="sm:col-span-2">
              ...
              <input className={INPUT} value={meta.title ?? ""} disabled={completed} onChange={(e) => setMetaField("title", e.target.value)} required />
            </div>
            <div className="sm:col-span-2">
              <label className={LABEL}>{t("Reason")} *</label>
              <input className={INPUT} value={meta.reason ?? ""} disabled={completed} onChange={(e) => setMetaField("reason", e.target.value)} required />
            </div>
            <div>
              <label className={LABEL}>{t("Start Date")} *</label>
              <input type="date" className={INPUT} value={meta.startDate ?? ""} disabled={completed} onChange={(e) => setMetaField("startDate", e.target.value)} required />
            </div>
            <div>
              <label className={LABEL}>{t("End Date")} *</label>
              <input type="date" className={INPUT} value={meta.endDate ?? ""} disabled={completed} onChange={(e) => setMetaField("endDate", e.target.value)} required />
            </div>
          </div>
        </section>
              ...
              {objectives.map((o) => (
                <div key={o._key} className="grid grid-cols-1 items-end gap-2 rounded-md border border-border/70 bg-secondary/20 p-2.5 md:grid-cols-[1fr_140px_130px_90px_auto]">
                  <div>
                    ...
                    <input className={INPUT} value={o.description ?? ""} disabled={completed} onChange={(e) => updateObjective(o._key, { description: e.target.value })} required />
                  </div>
                  <div>
      ...
      {/* Record outcome — only for a saved, not-yet-completed plan (HC135) */}
      {id && !completed && (
        <section className="rounded-lg border border-border bg-card p-4">
          <h3 className="mb-3 text-sm font-semibold">{t("Record Outcome")}</h3>
