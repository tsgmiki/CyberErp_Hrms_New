...
import { getAllProviderPayments, markProviderPaymentPaid, downloadProviderPaymentCsv } from "@/services/admin/trainingProviderPayment";
...
/** HC202 — training-provider payments handed off to finance (no finance module exists). */
function TrainingProviderPayment() {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [payFor, setPayFor] = useState<TrainingProviderPaymentModel | null>(null);
  const [reference, setReference] = useState("");
  const [busy, setBusy] = useState(false);
  const [actionMsg, setActionMsg] = useState("");

  const list = useEntityList({
    queryKey: "trainingProviderPayments",
    fetchPage: getAllProviderPayments,
  });

  const confirmPay = async () => {
    if (!payFor?.id) return;
    setBusy(true);
    const res = await markProviderPaymentPaid(payFor.id, reference || undefined);
    setBusy(false);
    setActionMsg(res.message);
    if (res.ok) {
      setPayFor(null);
      setReference("");
      queryClient.invalidateQueries({ queryKey: ["trainingProviderPayments"] });
    }
  };

  const exportCsv = async () => {
    try {
      await downloadProviderPaymentCsv();
    } catch {
  ...
  const columns = useMemo(
    () =>
      [
        { name: "providerName", label: "Provider", sort: true, render: (v: string) => <span className="font-semibold">{v}</span> },
        { name: "courseName", label: "Course", render: (v: string) => v || "—" },
        { name: "sessionStartDate", label: "Session", render: fmtDate },
        {
          name: "amount",
          label: "Amount",
          sort: true,
        ...
        { name: "reference", label: "Reference", render: (v: string) => v || "—" },
        {
          name: "Action",
          label: "Action",
          render: (_t: unknown, record: TrainingProviderPaymentModel) =>
    ...
    <div className="flex h-full min-h-0 flex-col p-3">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-base font-semibold text-foreground">{t("Provider Payments")}</h1>
            <p className="text-xs text-muted">{t("Training providers awaiting payment — raised automatically when their sessions complete.")}</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        <EntityListShell listKey="trainingProviderPayments" listLabel="Provider Payments" columns={columns} {...list} />
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-lg border border-border bg-background p-4 shadow-xl">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-foreground">{t("Mark payment as paid")}</h3>
              <button type="button" onClick={() => setPayFor(null)} className="rounded p-1 text-muted hover:bg-secondary/40"><X size={16} /></button>
            </div>
            <p className="mb-3 text-xs text-muted">
              {payFor.providerName} — {payFor.courseName} · <span className="font-semibold text-foreground">{Number(payFor.amount ?? 0).toLocaleString()}</span>
            </p>
...
export default memo(TrainingProviderPayment);
