...
import saveCompetencyCategory from "@/services/admin/competencyCategory/save";
import getCompetencyCategory from "@/services/admin/competencyCategory/get";
import Loading from "../../common/loader/loader";
import { activeStatusOptions, activeId, activeLabel } from "@/constants/orgStructure";

const FormProvider = memo(FormProviders);

function CompetencyCategoryForm(props: { id: string; setId: (id: string) => void }) {
  const { id, setId } = props;

  const [formState, setFormState] = useState<any>({});
  const [isLoading, setIsLoading] = useState(false);
  const [formData, setFormData] = useState({} as CompetencyCategoryModel);
  ...
  useEffect(() => {
    if (!id) setFormData({} as CompetencyCategoryModel);
  ...
  const { data: record, isLoading: pending } = useQuery({
    queryKey: ["competencyCategory", id],
    queryFn: () => getCompetencyCategory(id),
  ...
  const submitHandler = async (e: any) => {
    ...
    setIsLoading(true);
    const result = await saveCompetencyCategory(fd);
    setFormState(result);
    setIsLoading(false);
  };
  ...
  useEffect(() => {
    if (formState.status == "success") {
      setFormData({} as CompetencyCategoryModel);
      ...
      // ["competencyCategory", id] kept the pre-save copy and the client's 30 s staleTime served it to the
      // next Edit WITHOUT refetching -- grid fresh, form stale until a full page reload.
      queryClient.invalidateQueries({ queryKey: ["competencyCategory"] });
      setId("");
    }
  }, [formState]);
    ...
    <div className="text-white">
      ...
      <FormProvider
        ...
        form={{
          ...
          SubmitButton: "top",
          components: [
            { name: "name", label: "Name", placeholder: "e.g. Leadership", required: true, value: formData.name, onChange: changeHandler, error: formState?.zodErrors?.name, type: "text" },
