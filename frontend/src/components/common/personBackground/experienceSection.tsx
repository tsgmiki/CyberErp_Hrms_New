...
import type { EmployeeExperienceModel, FormComponentModel } from "@/models";
...
/**
 * Employment-history collection — shared by the Employee and Candidate modules (they differ only in
 * the `ds` adapter). Identical fields, columns, the External/Governmental toggle rows, dynamic custom
 * fields (HC021, OwnerType=Experience) and attachments in both places, by construction.
 */
function ExperienceSection({ ds }: { ds: BackgroundDataSource<EmployeeExperienceModel> }) {
          ...
          form={{
            ...
            components: [
              ...
              { name: "responsibilities", label: "Responsibilities", value: formData.responsibilities, onChange: changeHandler, type: "textarea", colSpan: "full" },
              {
                ...
                // `name`, so they post through the form's FormData like any field.
