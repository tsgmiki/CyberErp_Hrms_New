...
/** Bespoke JSON save (the `developmentActions[]` + `knowledgeTransfers[]` child arrays). */
export async function saveSuccessionCandidate(payload: SuccessionCandidateModel): Promise<SaveResult> {
  const isUpdate = !!payload.id;
  try {
    const res = await fetch(`${API_BASE_URL}/SuccessionCandidate`, {
