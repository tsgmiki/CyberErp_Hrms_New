...
**Multi-tenancy (Finbuckle.MultiTenant).** `HrmsDbContext : MultiTenantDbContext` — tenant isolation is enforced by Finbuckle (`[MultiTenant]` entities), resolved per-request via `HybridTenantStrategy` + `DatabaseTenantStore`. Every tenant-scoped entity carries `TenantId`; do not bypass the context to query across tenants. **Schema/table naming (after the 2026-08 module-schema rename): every table lives in its MODULE's schema, with no name prefix.** HRMS tables are `Hrms.Employee`, `Hrms.Position`, `Hrms.LookUpCategory`, … (configs use `ToTable("X", "Hrms")`); shared tables are `Core.Module`, `Core.Operation`, `Core.Subsystem`, `Core.Person`, `Core.SalaryScale`, `Core.Step`, `Core.Notification`, plus the already-unprefixed `Core.User` / `Core.Role` / `Core.Tenant` / `Core.RolePermission` / `Core.UserRole` / `Core.FiscalYear` / `Core.SubscriptionPlan` / `Core.TenantSubscription`. `HasDefaultSchema("Core")` still applies to the handful of entities without an explicit `ToTable`. The report **stored procedures are `Hrms.Report_X`** (renamed with the tables). HangFire's schema and both `__EFMigrationsHistory` tables are untouched.

> The prefixed names (`dbo.hrmsX`, `coreX`, `lupX`, `Core.hrms_*`) are GONE — older docs and migrations still show them and are accurate only for their date. `Core.Notification` is owned by the **Home** app (`ExcludeFromMigrations` here), so a rename affecting it needs both repos deployed together. When renaming tables again, see `memory.md` §4 for the four traps that cost a day last time (no `GO` between EF `Sql()` operations, proc names stored as data in bare *and* `[bracketed]` form, names built by string concatenation, and hand-written SQL outside the EF mappings).
