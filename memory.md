...
~~dropping `Subsystem.Url` would **break the Home launcher**~~ (**RESOLVED 2026-08-16 — see below**), and `User.CreatedAt` **REGRESSED in SRMS**
(I made it NOT NULL 08-14 — fix there). **The ~28 default-constraint diffs are DONE 2026-08-15** (handoff 0115) ⇒ **65 → 23**. ⚠️ Three needed HAND-WRITTEN SQL: `Role.Code` had an `(N'')` default from an older migration the MODEL NEVER DECLARED (EF cannot drop what it did not declare), `Subsystem.Code` was pure spelling (EF emits `N''`, SRMS stores `''`), and `CanExport` the reverse — **`HasDefaultValue(false)` emits NOTHING** because false is the CLR default. A hand-authored migration also needs an explicit `[Migration("id")]` attribute or EF never finds it. NOTE: these defaults are DECORATIVE — EF supplies every value on insert. **`Core.Tenant` + safe platform drops DONE 2026-08-15** (handoff 0116) ⇒ **23 → 18**: Tenant.TenantId was meaningless (the row IS the tenant; already in IsGlobalEntity; all 3 rows empty Guid) ⇒ **Tenant ZERO**; SubscriptionPlan/SubscriptionPlanModule/OrganizationSubscription (all **0 rows**, platform not tenant data) dropped theirs — ⚠️ the latter two added to `IsGlobalEntity` in the SAME change or reads 409. ⚠️ **NOT everything absent upstream is surplus:** `TenantSubscriptionAddOn.SubscribedTenantId` is a REAL FK to Core.Tenant (SRMS has a gap) and `Organization.TenantId` holds a REAL value — both left. **A column is surplus only when absent upstream AND carrying nothing here.** **⚠️ THE AUDIT HAD A HOLE (2026-08-15, handoff 0117): it compared COLUMNS ONLY — never FOREIGN KEYS or INDEXES.** `FK_Tenant_LookUpCategoryList` (in SRMS, absent in CERP) was invisible to every count reported. Added: `Tenant.TenantTypeId` → `Core.LookUpCategoryList`, SRMS constraint name, Restrict (all 3 rows NULL ⇒ safe). SRMS already had it. **FKs diff to ZERO both directions.** **INDEXES compared 2026-08-15** (handoff 0118): ⚠️ **the first run reported "0 differences" and was FALSE** — the FOR XML PATH query returned nothing, the `Where-Object {$_ -match}` filter swallowed it, and two EMPTY dictionaries compare as identical. **ALWAYS assert the harness loaded rows before trusting a zero.** Real answer **69**: 6 PK NAMES (SRMS uses PK_NavigationModule / PK_StandardRoleTemplate / PK_SystemSetting / PK_TenantNavigationModule / PK_TenantModuleEntitlement, and **`PK_Module` for Core.SUBSYSTEM**) → renamed via **`sp_rename`** because EF scaffolds rename as drop+add and SQL Server REFUSES to drop a PK that FKs reference; ⚠️ order matters (free `PK_Module` from Core.Module first). 4 alternate keys added. **53 CERP-only indexes KEPT** — performance (`IX_User_UserName`) + unique business keys; dropping regresses perf/integrity. ⇒ **every index/key SRMS has now exists in CERP identically.**
⚠️⚠️ **RETRACTION 2026-08-15 (handoff 0119): the two earlier "FKs diff to ZERO" claims were FALSE.** The FK query concatenated `fk.name` with `delete_referential_action_desc` — **different COLLATIONS** — so SQL Server errored, the `Where-Object {$_ -match}` filter ate the error, and two EMPTY dicts compared as identical. **SECOND false zero this session from the same shape: a harness returning nothing is indistinguishable from one that passes.** Fix: `COLLATE DATABASE_DEFAULT` + **always print/assert the load count before trusting a diff count**. Real number **13 → 9**: renamed `FK_TenantRolePermission_Operation_OperationId` (SRMS names a column `OperationId` that exists on NEITHER side) + `FK_TenantNavigationOperation_TenantModule_ModuleId`; added `LoginTrail.UserId→User` SET NULL and `TenantModule.TenantId→Tenant` (raw SQL — EF cannot model an FK on the value-converted TenantId). **ALL 3 DIMENSIONS RE-VERIFIED 2026-08-15** with `backend/scripts/compare-schemas.ps1` — one script that **ASSERTS ITS LOAD COUNTS before reporting**, so a zero is only trusted when the query provably read rows. Verified: **COLUMNS 18** (loaded 441/429), **FKs 9** (32/27), **INDEXES 53** (87/34, every one a CERP-only EXTRA — 0 missing, 0 mismatched). Earlier column + index results CONFIRMED; only FKs had been wrong. Script fixes both root causes: `COLLATE DATABASE_DEFAULT` on concatenated sys columns, `STRING_AGG` not `FOR XML PATH`. ⚠️ **Script is ASCII-only on purpose** — Windows PowerShell 5.1 reads a BOM-less UTF-8 .ps1 as ANSI, turning an em-dash into `â€”` and breaking the parse. **Run it before claiming parity.** Remaining 9 FKs are judgment: CERP-only domain FKs (JobGrade/Employee) KEEP; ⚠️ SRMS points `SubscriptionPlanModule.ModuleId` at **SubSystem** (rename artifact) — do NOT match; 2 CASCADE-vs-NO_ACTION are behavioural. ⚠️ **CERP has TWO lookup systems** — `Core.LookUpCategory/List` (SRMS platform mirror) vs `Hrms.LookUpCategory/List` (HRMS domain: education level/field of study, and what the `LookupCategoryList` ENTITY maps). Both empty, neither previously FK-referenced. Mapping the FK via EF silently scaffolded it against **Hrms** — the WRONG table — so it went in as RAW SQL. **When two tables share a name across schemas, READ the scaffold before applying.** The 18 column diffs remaining are load-bearing.
**`Core.Subsystem` FULLY IDENTICAL 2026-08-16** (logic §12.21) ⇒ **COLUMNS 12 → 10, INDEXES 53 → 52, FKs unchanged 9**. Dropped the last three CERP-only items: `SortOrder`, `Url`, and unique `IX_Subsystem_Name`. **`SortOrder` was a duplicate of `DisplayOrder`** — the §12.16 blocker (DisplayOrder all zeros while SortOrder held the real 0–5) is GONE because DisplayOrder is now curated, so no data migration was needed; everything orders by `DisplayOrder`. **`Url` moved to CONFIG, not deleted** — subsystem app addresses are per-ENVIRONMENT while these rows are SHARED across environments, so one column can never hold dev+staging+prod. Now `VITE_SUBSYSTEM_APPS`, a JSON `{code: appUrl}` map keyed by `Code` (case-insensitive), read by BOTH SPAs; it is the sibling of Home's existing `VITE_SUBSYSTEM_APIS` (that = where a subsystem's SERVER is, this = where its SPA is). ⚠️ **Wire contract UNCHANGED**: `getMySubsystems` stitches `url` back on from the registry, so launcher tiles / sidebar hrefs / `openExternalSubsystem` needed ZERO edits — **that service is the ONE swap point; resolve via `appUrlFor(code)`, never expect a server-supplied url.** ⚠️ `IX_Subsystem_Name` was KEPT DELIBERATELY at §12.16 (guards the duplicate-HOME bug that needed `dedup-subsystem-rows.sql`) — parity means it's gone and **nothing in the DB prevents a duplicate name now**; exposure is lower only because per-tenant rows are gone and HRMS's Subsystem module is read-only (§12.17), so a duplicate needs a manual insert. ⚠️⚠️ **`dotnet ef ... --no-build` LIES about pending model changes**: `database update --no-build` reported "model has pending changes" right after adding that exact migration, and a probe migration re-emitted the SAME ops — looking exactly like the known tools-9.0.5/runtime-10.0.8 drift and inviting the DriftProbe workaround. It was neither: `--no-build` leaves the OLD snapshot compiled in, so EF compares against a stale one and the probe "confirms" the false diff. **The tell: `migrations remove` named the PREVIOUS migration as the last one** (EF couldn't see the new files at all). **Drop `--no-build` and rebuild first**; use DriftProbe only after a real build rules this out. Verified by executing EF's own query against the live DB (throwaway probe console app, since the Home read early-returns for tenants with no projected modules): generated SQL selects `DisplayOrder`, references neither dropped column, 7 rows returned.
